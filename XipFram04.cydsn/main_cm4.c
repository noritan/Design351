...
void InitializeQpiMode(
    ...
    // Write CR2 for QPI mode
    {
        uint8_t CR2_Address[3] = {0x00, 0x00, 0x03};  // address 0x000003
        uint8_t CR2_Parameter[1] = {0x40};  // Enter to QPI mode
        
        // Try SPI mode
        SMIF_Mem_CmdSpiWriteAnyReg(slotNumber, CR2_Address, CR2_Parameter, sizeof CR2_Parameter, 0);
    }

    // Write CR1 for Memory Latency Code
    {
        uint8_t CR1_Address[3] = {0x00, 0x00, 0x02};  // address 0x000002
        uint8_t CR1_Parameter[1] = {(MLC<<4)};  // Set MLC parameter
        
        SMIF_Mem_CmdQpiWriteAnyReg(slotNumber, CR1_Address, CR1_Parameter, sizeof CR1_Parameter, 0);
    }
    ...
    // Write CR5 for Register Latency Code
    {
        ...
        SMIF_Mem_CmdQpiWriteAnyReg(slotNumber, CR5_Address, CR5_Parameter, sizeof CR5_Parameter, 0);
    }
}
...
//    SMIF_Mem_CmdSpiWriteEnable(TARGET_SLOT);
    ...
    {
        uint32_t    baseAddress = CY15B104QSNQPI_SlaveSlot_2.baseAddress;
        uint32_t    size = CY15B104QSNQPI_SlaveSlot_2.memMappedSize;
